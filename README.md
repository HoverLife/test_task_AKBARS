...
pip install pandas numpy scikit-learn imbalanced-learn matplotlib xgboost lightgbm category_encoders kmodes scipy seaborn openpyxl

Запуск лучшей модели и сегментации клиентов 

python model_rf_BEST.py    <--- model

cd segmentation            <--- segmentation
python segmentation.py

Все результаты будут находиться в папке results
...
- **Очистка**: Удалены некоррелирующие признаки (ИД, СМЕНА_МЖ, ЗАГРАН, ИНОСТР.ЯЗЫК,
        АВТО, ОТРАСЛЬ, РАБОТА_ПО_НАПР, ОТДЕЛЕНИЕ, ГОРОД)
- **Новые признаки**:
  - Кредитная нагрузка = Сумма кредита / (Личный доход + 1)
  - Возрастные группы (18-25, 26-35 и т.д.)
  - Семейный статус (3 категории)
- **Обработка пропусков**: Замена отсутствующих доходов супругов на 0

* Метод: K-Prototypes (смешанные данные)
**K-Prototypes** выбран благодаря:
- Возможности работы со смешанными данными (числовые + категориальные)
- Устойчивости к outliers
- Интерпретируемости результатов
- Оптимальной производительности на средних объемах данных
...
| ----------------------------------- | --------------- | ---------- | ------------------ | ------------------ | --------------  | 
| Зрелые клиенты (низкий риск)        | 19.8%           | 787        | 45+                | Средний            | 12,000          |
| Молодые семьи (высокий риск)        | 23.7%           | 827        | 26-35              | Низкий             | 16,000          |
| Премиум-сегмент (надежные)          | 15.9%           | 1643       | 36-45              | Низкий             | 22,000          |
| Перекредитованные (критический риск)| 23.3%           | 1413       | 26-35              | Средний            | 10,000          |
| Студенты (минимальный доход)        | 20.2%           | 1340       | 18-25              | Низкий             | 9,000           |
...
1. **Применение модели**:
   ...
   * Рассчитать вероятности дефолта(факт его наступления) и скоринговый балл(вероятность дефолта).
